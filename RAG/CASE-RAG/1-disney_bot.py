...
def parse_docx_with_llm(file_path, max_chunk_size=512):
    doc = DocxDocument(file_path)
    full_text = []
    for paragraph in doc.paragraphs:
        full_text.append(paragraph.text)
    text = '\n'.join(full_text)
    #TODO: need tooling to do splitting for tbl and p.
    """使用LLM进行高级语义切片"""
    # 检查环境变量
...
{{
  "chunks": [
    {{"type":"text","content":"第一个切片内容"}},
    {{"type":"text","content":"第二个切片内容"}},
    ...
  ]
}}

如果内容的tag是<p>便使用type：text。如果内容是<tbl>则使用type:table。
...
def build_knowledge_base(docs_dir, img_dir):
    ...
    for filename in os.listdir(docs_dir):
        ...
        if filename.endswith(".docx"):
            ...
            #chunks = parse_docx_with_llm(file_path)
